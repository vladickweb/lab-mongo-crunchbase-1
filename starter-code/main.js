		...
		function mainMenu() {
			...
			rl.question('Type an option: ', (option) => {
				switch (option) {
					...
					db.collection('companies').find({$and: [{},{'relationships.person.permalink': 'david-ebersman'}]},{ 'name':1, _id: 0}).toArray((error, result) => {
						if (error){
							console.log(error)
							rl.question(`\nType enter to continue: `, (answer) => { mainMenu() })
						} else {
					...
					db.collection('companies').find({name: 'Facebook'},{ 'competitions.competitor.name':1, _id: 0}).toArray((error, result) => {
						if (error){
							...
							resultFinal = result[0].competitions
							for (let i = 0; i < resultFinal.length; i++) {
								console.log(resultFinal[i].competitor.name)
							}
							rl.question(`\nType enter to continue: `, (answer) => { mainMenu() })
						}
					})
					...
					// 17 .- How many companies that has "social-network" in tag-list and founded between 2002 and 2016 inclusive and has offices in New York
						db.collection('companies').find({$and: [{},{tag_list: /social-network/}, {founded_year: {$gte: 2002}}, {founded_year:{$lte: 2016}},{'offices.city': 'New York'}]},{ 'name':1, _id: 0}).count((error, result) => {
							if (error){
								console.log(error)
								rl.question(`\nType enter to continue: `, (answer) => { mainMenu() })
							} else {
